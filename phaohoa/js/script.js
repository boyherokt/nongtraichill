...
// Đây là một ví dụ điển hình bắt đầu từ dự án đơn giản
// và đã phát triển vượt xa dự kiến ban đầu. Có hơi cồng kềnh
// khi đọc/xử lý file đơn lẻ này, nhưng dù sao nó vẫn ở đây :)
...
const IS_HIGH_END_DEVICE = (() => {
	...
	//所以一台四核台式机有8个核心。我们将在那里Cài đặt一个更Cao的最小阈值。
...
//特殊的Không可见颜色(未呈现，因此Không在颜色贴图中)
...
//随机Pháo hoa chữ内容
const randomWords = ["VÙNG ĐẤT CHILL", "HugNef"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀", "90px");
});

// Trợ giúp toàn màn hình, sử dụng tiền tố Fscreen.
function fullscreenEnabled() {
	return fscreen.fullscreenEnabled;
}

// Lưu ý trạng thái toàn màn hình được đồng bộ với store, store nên là nguồn
// xác định ứng dụng có đang ở chế độ toàn màn hình không.
function isFullscreen() {
	return !!fscreen.fullscreenElement;
}

// Thử chuyển đổi chế độ toàn màn hình.
...
// Đồng bộ thay đổi toàn màn hình với store. Cần event listener vì người dùng có thể
// chuyển đổi trực tiếp qua trình duyệt, ta muốn phản ứng lại.
	...
	state: {
		...
		config: {
			...
			size: IS_DESKTOP
				? "3" // Desktop default
				: IS_HEADER
				? "1.2" //配置文件头默认值(Không必是int)
				: "2", //手机默认
			wordShell: true, //Pháo hoa chữ 默认为开启 若Không开启可修改为false
...
const helpContent = {
	shellType: {
		...
		body: "Chọn loại pháo hoa bạn muốn bắn. Chọn 'Ngẫu nhiên (Random)' để có trải nghiệm thú vị nhất!",
	},
	shellSize: {
		header: "Kích thước pháo hoa",
		body: "Pháo hoa càng lớn thì phạm vi nổ càng rộng, nhưng cũng yêu cầu thiết bị mạnh hơn. Pháo hoa lớn có thể làm thiết bị của bạn bị chậm.",
	},
	quality: {
		header: "Chất lượng",
		body: "Nếu hoạt ảnh không mượt, hãy thử giảm chất lượng. Chất lượng càng cao, số lượng tia lửa càng nhiều, nhưng có thể làm thiết bị bị chậm.",
	},
	skyLighting: {
		header: "Chiếu sáng bầu trời",
		body: "Khi pháo hoa nổ, nền sẽ được chiếu sáng. Nếu màn hình quá sáng, hãy chọn 'Tối' hoặc 'Không'.",
	},
	scaleFactor: {
		header: "Thu phóng",
		body: "Điều chỉnh khoảng cách với pháo hoa. Đối với pháo hoa lớn, nên chọn giá trị thu phóng nhỏ hơn, đặc biệt trên điện thoại hoặc máy tính bảng.",
	},
	wordShell: {
		header: "Pháo hoa chữ",
		body: "Bật để hiển thị pháo hoa dạng chữ. Ví dụ: Chúc mừng năm mới, Vạn sự như ý, v.v.",
	},
	autoLaunch: {
		header: "Tự động bắn pháo hoa",
		body: "Bật để tự động bắn pháo hoa. Nếu tắt, bạn phải nhấn vào màn hình để bắn.",
	},
	finaleMode: {
		header: "Bắn nhiều pháo hoa cùng lúc",
		body: "Tự động bắn nhiều pháo hoa cùng lúc (cần bật 'Tự động bắn pháo hoa').",
	},
	hideControls: {
		header: "Ẩn nút điều khiển",
		body: "Ẩn các nút ở đầu màn hình. Vẫn có thể mở cài đặt ở góc phải.",
	},
	fullscreen: {
	...
	longExposure: {
		header: "Giữ lại tia lửa pháo hoa",
		body: "Giữ lại vệt sáng của pháo hoa lâu hơn.",
	},
};
...
//看Không见的星星需要一个标识符，即使它们Không会被渲染——物理学仍然适用。
...
//获取随机外壳，Không包括处理密集型变体
//注意，只有在配置中选择了“随机”shell时，这才是随机的。
//还有，这Không创建烟花，只返回工厂函数。
	...
	COLOR_CODES_W_INVIS.forEach((color) => {
		...
		for (let i = stars.length - 1; i >= 0; i = i - 1) {
			...
			if (star.life <= 0) {
				...
				//如果星形Không是“heavy”，应用空气阻力。重的性质被用于壳彗星。
				if (!star.heavy) {
 ...
 *
 * @return {void}              Không返回任何内容；由“particleFactory”使用给定的数据。
 */
 ...
 *
 * spreadSize:      爆发的大小。
 * starCount: 要创建的星星数量。这是可选的，如果省略，它将被Cài设置为一个合理的数量。
 * starLife:
 * starLifeVariation:
 * color:
 * glitterColor:
...
class Shell {
	 ...
	 * @param {number} launchHeight 爆炸所在Cao度
	 */
	...
	burst(x, y) {
		...
		if (this.comet) {
			...
			//根据当前Kích thước pháo hoa和选定的(最大)Kích thước pháo hoaThu phóng爆炸声音。
			//拍摄选择的外壳尺寸无论选择的尺寸如何，听起来总是一样的，
			//但是小一点的炮弹自动发射的时候，声音会小一点。听起来Không太好
			//但是当给定的值太小时，我们Không是根据比例，而是
			//看大小差异，映射到一个已知好听的范围。
			//这个项目的语言由Nianbroken翻译成中文
			const maxDiff = 2;
...
const soundManager = {
	...
	playSound(type, scale = 1) {
		...
		// *all* the way back.
		// Sửa: Không kiểm tra simSpeed trên thiết bị di động
		if (!canPlaySoundSelector() || (!IS_MOBILE && simSpeed < 0.95)) {
			return;
		}
...
// Nếu là header CodePen thì không cần âm thanh, chỉ khởi tạo.
if (IS_HEADER) {
	init();
} else {
	// Cho phép trạng thái hiển thị, sau đó tải tài nguyên và khởi động ứng dụng.
	setLoadingStatus("Đang khởi động pháo hoa...");
	setTimeout(() => {
		// Chỉ tải soundManager
		var promises = [soundManager.preload()];

		// Sau khi soundManager tải xong thì gọi init
		Promise.all(promises).then(init, (reason) => {
			console.log("Tải tài nguyên thất bại");
			init();
