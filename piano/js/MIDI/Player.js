...
root.playing = false;
root.timeWarp = 1;
// Sustain support for playback (optional, controlled by host app)
// When enabled, noteOff events are delayed by sustainMsPlayback milliseconds.
root.autoSustainPlayback = false;
root.sustainMsPlayback = 0;
...
var startAudio = function (currentTime, fromCache) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		// Some MIDI files encode note-off as note-on with velocity 0
		var subtype = (event.subtype === 'noteOn' && event.velocity === 0) ? 'noteOff' : event.subtype;
		switch (subtype) {
			case 'noteOn':
			...
			case 'noteOff':
				if (MIDI.channels[channel].mute) break;
				note = event.noteNumber - (root.MIDIOffset || 0);
				// apply optional sustain to playback note-offs
				var extraMs = (root.autoSustainPlayback && root.sustainMsPlayback > 0) ? root.sustainMsPlayback : 0;
				eventQueue.push({
					event: event,
					source: MIDI.noteOff(channel, event.noteNumber, (currentTime + extraMs) / 1000 + ctx.currentTime),
					interval: scheduleTracking(channel, note, queuedTime + extraMs, offset, 128)
				});
				break;
			default:
				break;
